...
import { ArrowBigDownDash, CircleCheck, CircleGaugeIcon } from "lucide-react";
      ...
      <div className="flex flex-col md:max-w-6xl gap-10 md:gap-14 md:mx-auto px-4 pt-5">
        ...
        <div className="flex flex-col gap-4  text-start md:text-center">
          {query ? (
            ...
            <h1 className="text-xl font-extrabold md:text-4xl">Tasks List</h1>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-10 w-full">
            {searchedTodos && searchedTodos.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center gap-2 md:gap-4 text-center">
                <p className="text-lg font-bold text-center">No tasks found</p>
                <p className="text-xs md:text-sm text-center">
                  Create a new task to get started
                </p>
                <ArrowBigDownDash className="text-gray-200 cursor-pointer w-7 md:w-10 h-7 text-center animate-bounce ease-in-out  md:h-10" />
              </div>
            )}
          </div>
        </div>
